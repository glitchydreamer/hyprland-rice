#!/usr/bin/env bash
# ============================================================================
# setup-home.sh — recreates the *home-directory* half of the CachyOS + Hyprland +
...
#
#     bash ~/Documents/hyprland-rice/cachyos/setup-home.sh            # menu
#     bash ~/Documents/hyprland-rice/cachyos/setup-home.sh hyprland scripts
#     bash ~/Documents/hyprland-rice/cachyos/setup-home.sh --yes all
#     bash ~/Documents/hyprland-rice/cachyos/setup-home.sh --dry-run all  # preview
#
# Flags:
...
COMPONENTS=(
    ...
    "scripts|~/.local/bin helpers: select-monitors.sh, hdr-toggle, ros2-humble, moveit2-humble, vnc-server, remote, lerobot-verify, fastfetch-logo"
...
# --- Duplicate-cursor fix: VRR off ------------------------------------------
# On this CachyOS + NVIDIA box the "two cursors, one stuck at centre" artifact
# is killed by turning Variable Refresh Rate OFF. caelestia's base config ships
# misc { vrr = 1 }; this override (sourced LAST) wins, so VRR is globally off
# WITHOUT editing the caelestia-tracked tree. The generated monitor lines also
# carry `vrr, 0` belt-and-suspenders. (The Arch box used a CPU-cursor-buffer
# workaround instead; vrr=0 is the cleaner fix that worked here.)
misc {
    vrr = 0
}
...
EOF
    ...
    # Restore the caelestia-tracked base misc.conf if it was hand-edited (the user
    # set vrr=0 there directly while exploring). The durable vrr=0 now lives in the
    # override above, so the base file should stay pristine — otherwise a future
    # `git pull` of the caelestia dotfiles conflicts on it. Only acts when the
    # caelestia tree is a git repo AND that one file shows as modified.
    local cae_tree="$HOME/.local/share/caelestia"
    local misc_rel="hypr/hyprland/misc.conf"
    if [ -d "$cae_tree/.git" ] && \
       ! git -C "$cae_tree" diff --quiet -- "$misc_rel" 2>/dev/null; then
        say ">>> Reverting hand-edit of caelestia's base $misc_rel (vrr now lives in"
        say "    the override). Keeps the caelestia tree clean for future git pulls."
        git -C "$cae_tree" checkout -- "$misc_rel" 2>/dev/null \
            || say "    !! couldn't auto-revert — do it manually: git -C $cae_tree checkout -- $misc_rel"
    fi
}
...
do_scripts() {
    dry "$BIN/{select-monitors.sh,hdr-toggle,ros2-humble,moveit2-humble,vnc-server,remote,lerobot-verify,fastfetch-logo}" && return
    mkdir -p "$BIN"
...
EOF
    ...
    # (No dualsense-audio helper on CachyOS — its stock PipeWire drives the
    # controller's 3.5mm jack correctly out of the box, so the Arch profile-routing
...
EOF
    ...
    chmod +x "$BIN/select-monitors.sh" "$BIN/hdr-toggle" "$BIN/ros2-humble" "$BIN/moveit2-humble" "$BIN/vnc-server" "$BIN/remote" "$BIN/lerobot-verify" "$BIN/fastfetch-logo"
}
